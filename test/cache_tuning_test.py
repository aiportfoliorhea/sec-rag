...
#     print(f"\n\n=== {name} ===")
#     cache = SemanticCache(embedder=emb, threshold=0.0)
#     scores = []
#     for p in test_set["pairs"]:
#         s = SemanticCache._cosine(emb.encode(p["q1"]), emb.encode(p["q2"]))
#         scores.append((s, p["label"], p["id"]))

#     for s, label, pid in sorted(scores, reverse=True):
#         print(f"{s:.3f}  {label:4}  #{pid}")

from sentence_transformers import CrossEncoder
ce = CrossEncoder("cross-encoder/stsb-roberta-base")

pairs = [(p["q1"], p["q2"]) for p in test_set["pairs"]]
scores = ce.predict(pairs)

rows = sorted(zip(scores, [p["label"] for p in test_set["pairs"]],
                  [p["id"] for p in test_set["pairs"]]), reverse=True)
for s, label, pid in rows:
    print(f"{s:.3f}  {label:4}  #{pid}")